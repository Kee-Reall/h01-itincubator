...
describe('start',()=> {
    ...
    it('crate incorrect', async () => {
        ...
        expect(res1.body.errorsMessages).toEqual(expect.any(Array))
    })
    ...
    it('crate incorrect', async () => {
        const res1 = await request(app).post(videoURL).send({
            title: NaN,
            author: undefined,
            availableResolutions: {length: undefined}
        })
        expect(res1.status).toBe(httpStatus.badRequest)
    ...
    it('crate incorrect', async () => {
        const res1 = await request(app).post(videoURL).send({
            title: "NaN",
            author: "undefined",
            availableResolutions: {length: NaN}
        })
        expect(res1.status).toBe(httpStatus.badRequest)
    ...
    it('crate correct', async () => {
        const res1 = await request(app).post(videoURL).send({
            title: "NaN",
            author: "undefined",
            availableResolutions: ['P144', 'P720', "P1080"]
        })
